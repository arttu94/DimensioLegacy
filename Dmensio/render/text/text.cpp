	...
	XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, deviceContext, "../Dmensio/data/fontdata.txt", "../Dmensio/data/texture/font.tga");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, "abcdefghijklmnopqrstuvxyz1234567890", 10, 10, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
		return false;
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "ttest text 2", 10, 30, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
		return false;
...
bool Text::Render(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX orthoMatrix, XMMATRIX viewMatrix)
{
	bool result;
...
bool Text::RenderSentence(ID3D11DeviceContext* deviceContext, SentenceType* sentence, XMMATRIX worldMatrix,
	XMMATRIX orthoMatrix, XMMATRIX viewMatrix)
{
